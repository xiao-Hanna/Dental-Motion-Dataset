...
class CMUNeXt(nn.Module):
    def __init__(self, input_channel=3, num_classes=1, dims=[16, 32, 128, 160, 256], depths=[1, 1, 1, 3, 1], kernels=[3, 3, 7, 7, 7]):
        ...
        self.Conv_1x1 = nn.Conv2d(dims[0], num_classes, kernel_size=1, stride=1, padding=0)  # 这里正确使用了num_classes
    ...
    def forward(self, x):
        ...
        d2 = self.Up_conv2(d2)
        d1 = self.Conv_1x1(d2)  # 输出通道数为num_classes

        return d1


# 修正：将硬编码的num_classes=1改为传入的num_classes参数
def cmunext(input_channel=3, num_classes=1, dims=[16, 32, 128, 160, 256], depths=[1, 1, 1, 3, 1], kernels=[3, 3, 7, 7, 7]):
    return CMUNeXt(
        dims=dims,
        depths=depths,
        kernels=kernels,
        input_channel=input_channel,  # 同时修正input_channel的传递（原代码写死为3）
        num_classes=num_classes  # 关键修正：使用传入的num_classes
    )
...
def cmunext_s(input_channel=3, num_classes=1, dims=[8, 16, 32, 64, 128], depths=[1, 1, 1, 1, 1], kernels=[3, 3, 7, 7, 9]):
    return CMUNeXt(
        ...
        depths=depths,
        kernels=kernels,
        input_channel=input_channel,  # 修正input_channel传递
        num_classes=num_classes  # 关键修正
    )
